...
# shellcheck contrib/guix/libexec/check.bash
source contrib/guix/libexec/check.bash
    ...
    for binary in "build/bin"/*; do
        ...
        case "$HOST" in
            *linux-gnu*)
                ...
                allowed_dynamic_libraries=("libm.so.6" "libpthread.so.0" "libc.so.6" $(basename "$glibc_dynamic_linker"))
