...
check_string() {
    ...
    if strings "$binary" | grep -q "${string}"; then
        echo "Error: ${binary} contains unexpected string: \"${string}\""
        exit 1
    fi
}
...
check_rpath() {
    ...
    if [ ! -z "$rpath" ]; then
        echo "Error: ${binary} contains unexpected rpath: ${rpath}"
        exit 1
    fi
}
...
check_interpreter() {
    ...
    if [ "$interpreter" != "$allowed_interpreter" ]; then
        echo "Error: ${binary} contains unexpected interpreter: ${interpreter}, should be: ${allowed_interpreter}"
        exit 1
    fi
}
...
check_libraries() {
    ...
    for library in "${dynamic_libraries[@]}"; do
        ...
        if [ "$found" -eq 0 ]; then
            echo "Error: ${binary} links against unexpected library: ${library}"
            exit 1
        fi
    done
}
