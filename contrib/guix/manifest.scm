...
(define base-rust rust-1.88)

(define-public rust-std
  (package
    (name "rust-std")
    (version (package-version base-rust))
    ;; You'd expect (source (package-source (base-rust)) to work here,
    ;; but it refers to the source store item and NOT the .tar.gz archive
    (source (origin
              (method url-fetch)
              (uri (origin-uri (package-source base-rust)))
              (sha256
                (content-hash-value (origin-hash (package-source base-rust))))))
    (build-system trivial-build-system)
    ...
    (synopsis (package-synopsis base-rust))
    (description (package-description base-rust))
    (home-page (package-home-page base-rust))
    (license (package-license base-rust))))
