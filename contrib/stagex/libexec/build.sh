...
MONERO_SRCDIR="/monero"
STAGEX_DIR="${MONERO_SRCDIR}/stagex"

DEPENDS_SOURCES="${STAGEX_DIR}/depends-sources/"
mkdir -p "${DEPENDS_SOURCES}"
DEPENDS_CACHE="${STAGEX_DIR}/depends-cache/${STAGEX_ENVIRONMENT}"
mkdir -p "${DEPENDS_CACHE}"

VERSION_DIR="${STAGEX_DIR}/builds/${VERSION}"

LOGDIR="${VERSION_DIR}/logs/${HOST}"
mkdir -p "${LOGDIR}"

DISTSRC="${VERSION_DIR}/build/${HOST}"
rm -rf "${DISTSRC}" # rebuild if we already have this
mkdir -p "${DISTSRC}"

ACTUAL_OUTDIR="${VERSION_DIR}/output/${HOST}"
mkdir -p "${ACTUAL_OUTDIR}"
...
#printenv | sort | grep -v '^\(DISTNAME=\|DISTSRC=\|SOURCES_PATH=\|JOBS=\)' > ${LOGDIR}/stagex-env.txt
